...
int isKaprekar(int n) {
  ...
  if(n < 1) {
    return 0; //changed to 0, not false
  }

  int i;
  long square = (long) n * n;  //changed the order
  int numDigits = (int) log10(n) + 1;
  long modulus = 1;  //changed to modulus = 1
  long first, second;

  //for each possible "split" of the square...
  for(i=1; i<=numDigits + 2; i++) {  //changed to numdigits, added +2
    //increase the modulus by a factor of 10
    modulus *= 10;   //changed the misspelling of modulous to modulus
